...
class DiscordNotifier:
    ...
    def __init__(self, bot_token, channel_id, cooldown_seconds=60, bot_name='Shinobi SMTP Proxy'):
    ...
    async def send_alert(self, camera_id, description, jpeg_bytes=None):
        ...
        try:
            ...
            if jpeg_bytes:
                ...
                form.add_field('files[0]', io.BytesIO(jpeg_bytes),
                               filename='alert.jpg', content_type='image/jpeg')
                async with session.post(url, headers=self._headers, data=form) as resp:
                    if resp.status in (200, 201):
                        ...
                        log.warning("Discord API error %d for %s: %s", resp.status, camera_id, body)
                        return False
            else:
